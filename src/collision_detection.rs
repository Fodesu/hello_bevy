...
#[derive(Component, Debug)]
pub struct CollisionDeterctionPlugin;


impl Plugin for CollisionDeterctionPlugin {
...
fn collision_detection(mut query : Query<(Entity, &GlobalTransform, &mut Collider)>) {
    ...
    for (entity_a, transform_a, collision_a) in query.iter() {
        for (entity_b, transform_b, collision_b) in query.iter() {
